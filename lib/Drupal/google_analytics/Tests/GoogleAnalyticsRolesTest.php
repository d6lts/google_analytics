...
class GoogleAnalyticsRolesTest extends WebTestBase {
  ...
  function testGoogleAnalyticsRolesTracking() {
    ...
    config('google_analytics.settings')->set('visibility.roles_enabled', 0)->save();
    ...
    config('google_analytics.settings')->set('visibility.roles', array(DRUPAL_AUTHENTICATED_RID => DRUPAL_AUTHENTICATED_RID))->save();
    ...
    config('google_analytics.settings')->set('visibility.roles_enabled', 1)->save();
    // Enable tracking for all users.
    config('google_analytics.settings')->set('visibility.roles', array())->save();
